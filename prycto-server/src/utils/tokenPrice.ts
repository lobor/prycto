...
export class TokenPrice {
  static async calcSell(web3: Web3, tokenAddres: string) {
    ...
    const tokensToSellTmp = TokenPrice.setDecimals(tokensToSell, tokenDecimals);
    let priceTokenBnb;
    try {
      ...
      priceTokenBnb = await router.methods
        .getAmountsOut(tokensToSellTmp, [tokenAddres, BNBTokenAddress])
        .call();
      priceTokenBnb = Number(web3.utils.fromWei(priceTokenBnb[1]));
    } catch (error) {
      console.log(error);
    }

    if (!priceTokenBnb) return 0;
    const bnbPrice = await TokenPrice.calcBNBPrice(web3);
    return priceTokenBnb * bnbPrice;
  }

  static async calcBNBPrice(web3: Web3) {
    const BNBTokenAddress = '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c'; //BNB
  ...
  static setDecimals(number: number, decimals: number) {
    const numberTmp = number.toString();
    const numberAbs = numberTmp.split('.')[0];
    let numberDecimals = numberTmp.split('.')[1] ? numberTmp.split('.')[1] : '';
