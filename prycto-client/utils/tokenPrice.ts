...
// async function queryBlock(web3, i) {
//   var json = await web3.eth.getBlock(i);
//   return json;
// }

export class TokenPrice {
  static async history(web3: Web3, tokenAddress: string) {
    let tokenRouter = await new web3.eth.Contract(
      tokenAbi as any,
      tokenAddress
    );
    ...
    let block = await dater.getDate(
      new Date("2021-10-10T13:20:40Z"), // Date, required. Any valid moment.js value: string, milliseconds, Date() object, moment() object.
      true // Block after, optional. Search for the nearest block before or after the given date. By default true.
    );
    console.log(block);
    const toot = await web3.eth.getBlock(block.block);
    console.log(toot);
    console.log(await TokenPrice.calcSell(web3, tokenAddress, block.block));
    // var blocks = [];
    // web3.eth.getBlockNumber().then(async (n) => {
    //   console.log(n);
    //   for (let i = 0; i < 10; i++) {
    //     const block = await queryBlock(web3, i);
    //     console.log(new Date(block.timestamp * 1000));
    //   }
    // });
    // const events = await tokenRouter.getPastEvents("allEvents", {
    //   fromBlock: 0,
    //   toBlock: 'latest',
    // });
    // console.log(events);
  }
  static async calcSell(web3: Web3, tokenAddres: string, block?: string) {
    const tokensToSell = 1;
    const BNBTokenAddress = "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"; //BNB
    let tokenRouter = await new web3.eth.Contract(tokenAbi as any, tokenAddres);
    ...
    let bnbPrice = await TokenPrice.calcBNBPrice(web3, block);
    return priceTokenBnb * bnbPrice;
  }

  static async calcBNBPrice(web3: Web3, block?: string) {
    const BNBTokenAddress = "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"; //BNB
  ...
  static setDecimals(number: number, decimals: number) {
    ...
    let numberAbs = numberTmp.split(".")[0];
